...
class MyWindow:
    def __init__(self, win):
        self.window = win
        self.random_lbl= Label(win,
                                text='Generate\nStudentID number',
                                bg='LavenderBlush4',
                                font=("Times", 8, "bold", "italic"))
        self.random_lbl.place(x=790, y=40)

        self.Random_ent = Entry(
            bd=1,
            width=10,
        )
        self.Random_ent.place(x=795, y=75)
        self.random_bt = Button(win,
                              text=' Generate',
                              command=self.generate,
                              bg="slate grey")
        self.random_bt.place(x=795, y=100)

        self.use_bt = Button(win,
                              text=' use',
                              command=self.use,
                              bg="slate grey")
        self.use_bt.place(x=815, y=130)
        ...
        self.refresh_bt = Button(win,
                                 text='Refresh',
                                 command=self.refresh,
                                 bg="slate gray",
                                 height=5,
                                 width=5)
        self.refresh_bt.place(x=680, y=10)

        self.clear_bt = Button(win,
                               text='Clear',
                               command=self.clear,
                               bg="slate gray",
                               height=1,
                               width=5)
        self.clear_bt.place(x=680, y=110)
